...
public class Algorithms {
	...
	public static int alphaBeta(Evaluator evaluator, GameState gstate,
		...
		if (gstate.isTerminal() || depth == 0) {
			...
			for (GameState childState : gstate.getChildStates()) {
				int value = -alphaBeta(evaluator, childState, depth - 1, -beta,
						-alpha);
		...
		if (gstate.isTerminal() || depth == 0) {
			...
			for (GameState childState : gstate.getChildStates()) {
				bestValue = Math.max(bestValue,
						-miniMax(evaluator, childState, depth));
			}
		}
		return bestValue;
	}
	...
	// public static int alphaBeta2(Evaluator evaluator, GameState gstate, int
